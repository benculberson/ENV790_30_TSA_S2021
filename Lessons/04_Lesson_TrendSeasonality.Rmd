...
nhydro <- ncol(raw_inflow_data)-2 #the number of columns in this data set that have data, the first two are dates
...
my_date <- paste(raw_inflow_data[,1],raw_inflow_data[,2],sep="-") #remember we start indexing at 1 cuz we're assholes
my_date <- my(my_date)  #function my from package lubridate, my is short for month, year
head(my_date)
...
#this is going to be a loop, refer to this from now on when performing a loop
for(i in 1:nhydro){ #remember nhydro has 15 columns so it has the value of 15
  print(ggplot(inflow_data, aes(x=my_date, y=inflow_data[,(1+i)])) +
            geom_line(color="blue") +
            ylab(paste0("Inflow ",colnames(inflow_data)[(1+i)],sep="")) #naming the labels using a loop as well, the separator is my choice (the "" are not required")
        )
}
...
#update object with number of observations, just keeping things in order
nobs <- nobs-4
...
```{r}
ts_inflow_data <- ts(inflow_data[,2:(2+nhydro-1)],frequency=12) #this should still be 15 columns frequency = 12 means this data is monthly (every 12 observations you go back to the same month)
...
iHP=4  #change this to check other HP
#prep_data <- data.frame("Inflow"=inflow_data[,iHP],"Time"=t)

#Fit a linear trend to TS of iHP
linear_trend_model=lm(inflow_data[,iHP+1]~t)  #left of tilda is dependent, to the right is independent. The reason we add 1 is because the first column is date
...
detrend_inflow_data <- inflow_data[,(iHP+1)]-(beta0+beta1*t) #remember we just set these values in the previous chunk

#Understanding what we did
ggplot(inflow_data, aes(x=my_date, y=inflow_data[,(1+iHP)])) + #[,(1+iHP)]) means all rows and the iHP column (remember that the 1st column is the date)
            geom_line(color="blue") +
            ylab(paste0("Inflow ",colnames(inflow_data)[(1+iHP)],sep="")) +
            #geom_abline(intercept = beta0, slope = beta1, color="red")
            geom_smooth(color="red",method="lm") +
...
#the green with the orange line is the detrended data
...
dummies <- seasonaldummy(ts_inflow_data[,iHP])  #only need 11 dummies, because we have 12 months and the 12th dummy is just a 0 for all of them
...
inflow_seas_comp=array(0,nobs) #just a vector of 0s that has the length of observations (just can be altered with the following loop), use vector or array for this job. A list would treat each 0 as a different object, whereas the arrray/vector would not
for(i in 1:nobs){
  inflow_seas_comp[i]=(beta_int+beta_coeff%*%dummies[i,])#   %*% just means inner product of two vectors (1st times 1st value, 2nd times 2nd value, and so on)
}
