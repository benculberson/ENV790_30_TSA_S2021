...
Rainfall_BR_processed <-
  Rainfall_BR %>% #this thing to the left is called a pipe operator, so everything with this will be done under the original data frame, we don't need to keep using $ 
...
BR_complete <- inner_join(Rainfall_BR_processed,Temp_BR_processed) #inner_join joins df by date
...
ggplot(BR_complete, aes(x=Date, y=BR_complete[,2])) + 
            geom_line(color="blue") +
            ylab(paste0(colnames(BR_complete)[(2)],sep=""))

ggplot(BR_complete, aes(x=Date, y=BR_complete[,3])) + 
            geom_line(color="blue") +
            ylab(paste0(colnames(BR_complete)[(3)],sep=""))
...
BR_complete_ts<-ts(BR_complete, frequency = 12, start = 1901)
...
Rainfall_ts<-BR_complete_ts[,2]
Temperature_ts<-BR_complete_ts[,3]

stl(Rainfall_ts, s.window = "period")
stl(Temperature_ts, s.window = "period")
...
#you run this one the ts() object that we've created, not the stl() or the original data
